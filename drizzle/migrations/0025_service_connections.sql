...
-- Date: 2026-05-15
--
-- Adds host-owned service connection configuration. Plugins declare service
-- requirements in plugin.ts; only the host stores base URLs, secret refs, and
...
CREATE TABLE IF NOT EXISTS "host_secrets" (
  ...
  "encoding" text NOT NULL DEFAULT 'aes-256-gcm-v1',
  "created_by_user_id" text,
  "created_at" timestamptz NOT NULL DEFAULT now(),
  "updated_at" timestamptz NOT NULL DEFAULT now()
);
--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS "host_secrets_namespace_name_idx"
  ON "host_secrets" ("namespace", "name");
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "host_secrets_namespace_idx"
  ON "host_secrets" ("namespace");
--> statement-breakpoint
ALTER TABLE "host_secrets" ENABLE ROW LEVEL SECURITY;
--> statement-breakpoint
ALTER TABLE "host_secrets" FORCE ROW LEVEL SECURITY;
--> statement-breakpoint
DROP POLICY IF EXISTS host_secrets_system_only ON "host_secrets";
--> statement-breakpoint
CREATE POLICY host_secrets_system_only ON "host_secrets"
FOR ALL
USING (current_app_user_id() = 'system')
WITH CHECK (current_app_user_id() = 'system');
--> statement-breakpoint
CREATE TABLE IF NOT EXISTS "plugin_service_connections" (
  "id" text PRIMARY KEY,
...
CREATE UNIQUE INDEX IF NOT EXISTS "plugin_service_connections_global_default_idx"
  ON "plugin_service_connections" ("plugin_id", "service_name")
...
CREATE UNIQUE INDEX IF NOT EXISTS "plugin_service_connections_global_environment_idx"
  ON "plugin_service_connections" ("plugin_id", "service_name", "environment")
...
CREATE UNIQUE INDEX IF NOT EXISTS "plugin_service_connections_workspace_default_idx"
  ON "plugin_service_connections" ("plugin_id", "service_name", "scope_id")
...
CREATE UNIQUE INDEX IF NOT EXISTS "plugin_service_connections_workspace_environment_idx"
  ON "plugin_service_connections" ("plugin_id", "service_name", "scope_id", "environment")
...
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "plugin_service_connections_scope_lookup_idx"
  ON "plugin_service_connections" ("scope_type", "scope_id", "environment");
