...
test('module-safe UI SDK exposes ordinary page and resource primitives', () => {
  const page = (
    <Page>
      <PageHeader title="Notes" actions={<Button>New</Button>} />
      <Section title="Recent">
        <ResourceTable
          rows={[{ id: 'note_1', title: 'First note' }]}
  ...
  assert.equal(React.isValidElement(page), true);
  assert.equal(
    React.isValidElement(<ResourceTable rows={[]} columns={[{ key: 'title', header: 'Title' }]} />),
    true
  );
});
