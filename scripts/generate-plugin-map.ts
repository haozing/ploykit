...
function generatedHeader(plugins: PluginInfo[]): string {
  return `/**
...
function generatePluginMap(
  plugins: PluginInfo[],
  outputFile: string,
  format: 'typescript' | 'commonjs' = 'typescript'
): string {
  ...
  if (format === 'commonjs') {
    return `${generatedHeader(plugins)}
const PLUGIN_MAP = {
${entries.join(',\n')}
};
...
function assertRuntimeMapFileIsLoadable(files: PluginMapFileSet): void {
  if (!files.runtimeArtifact || isLoadableRuntimePluginMapFile(files.mapFile)) {
    return;
  }

  throw new Error(
    `Runtime plugin map must be a loadable CommonJS artifact (.cjs or .js): ${path.relative(
      PROJECT_ROOT,
      files.mapFile
    )}`
  );
...
function main() {
  ...
  if (ACTIVE_FILES.runtimeArtifact) {
    assertRuntimeMapFileIsLoadable(ACTIVE_FILES);
  }
  ...
  const sourceManifestContent = sourcePlugins
    ...
    : '';
  const activeContent = generatePluginMap(
    activePlugins,
    ACTIVE_FILES.mapFile,
    ACTIVE_FILES.runtimeArtifact ? 'commonjs' : 'typescript'
  );
