...
Use host behavior through `ctx.*` and declare matching permissions.
...
- Secrets belong in `ctx.secrets`, not config, source files, or environment variables.
- Long-running work belongs in `ctx.runs` and jobs, not request/response handlers.
