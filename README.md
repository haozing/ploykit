...
Modules are workspace source modules under `modules/<module-id>/`. The default
configuration points at the workspace `modules` directory:
...
Every module root must contain `module.ts`. PloyKit no longer supports loading
module source from outside this repository; keep product module code under
`modules/<module-id>/`. External services remain supported through
`serviceRequirements`, `ctx.services.invoke(...)`, OpenAPI contracts, and module
owned live-smoke tests.

After editing modules or entry points, regenerate the runtime registry:
...
- `src/lib/module-map.manifest.json`: module hashes, release evidence, and quality declarations.
...
模块源码放在工作区 `modules/<module-id>/` 下。默认配置指向工作区 `modules` 目录：
...
每个模块根目录必须包含 `module.ts`。PloyKit 不再支持从仓库外加载模块源码；产品模块代码应放在
`modules/<module-id>/`。外部服务仍通过 `serviceRequirements`、`ctx.services.invoke(...)`、OpenAPI 契约和模块自有 live smoke 接入。

修改模块或模块入口后，重新生成运行时注册表：
...
- `src/lib/module-map.manifest.json`：模块哈希、发布证据和质量声明。

## 运行时边界

- 产品专属代码应放在仓库内 `modules/<module-id>/`。
...
- 服务端分离型模块应从 `product --with service-backed` 开始，用 `module:service-contract --write-fixtures` 维护 contract mock，用模块自有 live smoke 证明真实签名、租户、幂等、quota 和状态机行为。
...
欢迎贡献。请先阅读 [CONTRIBUTING.md](CONTRIBUTING.md)，将产品专属改动保持在仓库内 `modules/<module-id>/`，并在提交 PR 前运行相关验证命令。
