import { DashboardOrdersOperationsPage } from '@host/components/dashboard/DashboardPages';
...
export default async function OrdersPage({ params }: { params: Promise<LanguageRouteParams> }) {
  const [lang, session] = await readLanguageAndRequireUser(params, '/dashboard/orders');
  const snapshot = await getUserSaasSnapshot(session);
  return renderPresentedHostPage({
    pageId: 'dashboard.orders',
    defaultPage: <DashboardOrdersOperationsPage lang={lang} snapshot={snapshot} />,
    lang,
    session,
...
export async function generateMetadata({ params }: { params: Promise<LanguageRouteParams> }) {
  ...
  return generatePresentedHostMetadata({ pageId: 'dashboard.orders', lang });
}
