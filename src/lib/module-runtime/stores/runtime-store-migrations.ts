...
export const RUNTIME_STORE_REQUIRED_TABLES = [
  ...
  'module_host_users',
  'module_platform_users',
  'module_workspace_members',
  'module_workspace_invites',
  'module_auth_sessions',
  ...
  'module_idempotency_keys',
  'module_rate_limit_events',
...
const REQUIRED_COLUMNS: Record<string, readonly string[]> = {
  ...
  module_platform_users: ['id', 'email', 'status', 'metadata', 'created_at', 'updated_at'],
  module_workspace_members: [
    'id',
    'product_id',
    'workspace_id',
  ...
  module_rate_limit_events: ['id', 'bucket', 'cost', 'occurred_at', 'expires_at'],
  module_credit_reservations: [
    'id',
    'product_id',
    'environment_id',
...
export const RUNTIME_STORE_REQUIRED_INDEXES: readonly RuntimeStoreIndexAuditEntry[] = [
  ...
  },
  {
    ...
    domain: 'runs',
    table: 'module_runs',
    query: 'run idempotency by product/environment/workspace/module/key',
    columns: ['product_id', 'environment_id', 'workspace_id', 'module_id', 'idempotency_key'],
    unique: true,
  },
  {
    ...
    domain: 'outbox',
    table: 'module_outbox',
    query: 'outbox idempotency by product/environment/workspace/name/key',
    columns: ['product_id', 'environment_id', 'workspace_id', 'name', 'idempotency_key'],
    unique: true,
  },
  {
  ...
  },
  {
    ...
    columns: ['product_id', 'environment_id', 'workspace_id', 'user_id', 'idempotency_key'],
    unique: true,
  },
  {
  ...
  },
  {
    ...
    domain: 'identity',
    table: 'module_api_keys',
    query: 'API key inventory by product/environment/workspace/module/status',
    columns: ['product_id', 'environment_id', 'workspace_id', 'module_id', 'status'],
  },
  {
  ...
  {
    index: 'module_user_identities_provider_key_uidx',
    domain: 'identity',
    table: 'module_user_identities',
    query: 'user identity uniqueness by product/environment/provider/key',
    columns: ['product_id', 'environment_id', 'provider', 'provider_key'],
    unique: true,
  },
  {
    index: 'module_user_identities_user_lookup_idx',
    domain: 'identity',
    table: 'module_user_identities',
    query: 'user identity inventory by hosted user and provider',
    columns: ['product_id', 'environment_id', 'user_id', 'provider', 'status'],
  },
  {
    index: 'module_platform_users_email_uidx',
    domain: 'identity',
    table: 'module_platform_users',
    query: 'platform user lookup by normalized email',
    columns: ['email'],
    unique: true,
  },
  {
    index: 'module_platform_users_status_idx',
    domain: 'identity',
    table: 'module_platform_users',
    query: 'platform user inventory by status',
    columns: ['status', 'created_at'],
  },
  {
    index: 'module_workspace_members_scope_uidx',
    domain: 'identity',
    table: 'module_workspace_members',
    query: 'workspace membership uniqueness by platform user',
    columns: ['product_id', 'workspace_id', 'platform_user_id'],
    unique: true,
  },
  {
    index: 'module_workspace_members_lookup_idx',
    domain: 'identity',
    table: 'module_workspace_members',
    query: 'workspace member inventory by status and role',
    columns: ['product_id', 'workspace_id', 'status', 'role'],
  },
  {
    index: 'module_workspace_invites_token_uidx',
    domain: 'identity',
    table: 'module_workspace_invites',
    query: 'workspace invite lookup by token hash',
    columns: ['token_hash'],
    unique: true,
  },
  {
    index: 'module_workspace_invites_scope_idx',
    domain: 'identity',
    table: 'module_workspace_invites',
    query: 'workspace invite inventory by status and expiry',
    columns: ['product_id', 'workspace_id', 'status', 'expires_at'],
  },
  {
    index: 'module_auth_sessions_subject_idx',
    domain: 'identity',
    table: 'module_auth_sessions',
    query: 'auth session lookup by product/environment/workspace/subject',
    columns: [
      'product_id',
      'environment_id',
      'workspace_id',
  ...
  {
    index: 'module_auth_sessions_status_idx',
    domain: 'identity',
    table: 'module_auth_sessions',
    query: 'auth session cleanup by status and expiry',
    columns: ['status', 'expires_at', 'revoked_at'],
  },
  {
    index: 'module_idempotency_keys_scope_uidx',
    domain: 'idempotency',
    table: 'module_idempotency_keys',
    query: 'Stripe-style idempotency key by product/environment/workspace/namespace/key',
    columns: ['product_id', 'environment_id', 'workspace_id', 'namespace', 'idempotency_key'],
    unique: true,
  },
  {
    index: 'module_idempotency_keys_lookup_idx',
    domain: 'idempotency',
    table: 'module_idempotency_keys',
    query: 'idempotency replay and 24h GC lookup',
    columns: ['product_id', 'environment_id', 'workspace_id', 'namespace', 'status', 'expires_at'],
  },
  {
    index: 'module_idempotency_keys_locked_idx',
    domain: 'idempotency',
    table: 'module_idempotency_keys',
    query: 'in-progress idempotency lock recovery',
    columns: ['status', 'locked_at'],
  },
  {
    index: 'module_rate_limit_events_bucket_time_idx',
    domain: 'security',
    table: 'module_rate_limit_events',
    query: 'shared sliding-window rate limit by bucket/time',
    columns: ['bucket', 'occurred_at'],
  },
  {
    index: 'module_rate_limit_events_expiry_idx',
    domain: 'security',
    table: 'module_rate_limit_events',
    query: 'rate limit event retention cleanup',
    columns: ['product_id', 'environment_id', 'workspace_id', 'user_id', 'unit', 'status', 'expires_at'],
  },
  {
  ...
  },
  {
    ...
    columns: ['product_id', 'environment_id', 'workspace_id', 'user_id', 'status'],
  },
  {
  ...
  {
    index: 'module_credit_reservations_expiry_idx',
    domain: 'commercial',
    table: 'module_credit_reservations',
    query: 'reserved credit reservation expiry sweep',
    columns: ['product_id', 'environment_id', 'workspace_id', 'status', 'expires_at'],
  },
  {
  ...
  },
  {
    ...
    query: 'active risk block uniqueness by product/workspace/subject/scope',
    columns: ['product_id', 'workspace_id', 'subject_type', 'subject_id', 'scope', 'released_at'],
    unique: true,
  },
  {
    ...
    columns: ['product_id', 'workspace_id', 'subject_type', 'subject_id', 'released_at'],
  },
] as const;
