...
| 6 | dev/build 生命周期未写死，承诺"开发命令自动 watch"过于乐观 | `host:dev` 只做显式检查和启动提示；CI `modules:check` + `git diff` 守卫；不承诺 watch；Sidebar 加 fallback 防止 stale registry 时空白 | §5.4、§5.6、§9 |
