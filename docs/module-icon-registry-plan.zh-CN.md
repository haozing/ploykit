...
- SDK 增加 `resources.icons` 类型与 validator 基础校验，支持 `{ kind: 'lucide'; name }` 与 `{ kind: 'svg'; path }`。
...
- RunLynk 当前按模块侧短期方案使用 host core 图标 `activity`；`resources.icons.clipboardList` 迁移留给后续独立模块 PR。
...
- 与 `resources.locales` 心智对称，模块作者无新概念负担。
- 宿主**只改一次**，后续模块迁入零侵入。
...
与 `resources.locales` 对称，新增 `resources.icons`：
...
模块作者在 `resources.icons` 与 `navigation[].icon` 中只写**模块内本地 key**（如 `clipboardList`、`workerHat`）。
...
1. `@ploykit/module-sdk` 增加 `resources.icons` schema 与 validator 规则（§6）。
...
1. RunLynk：在 `module.ts` 加 `resources.icons.clipboardList = { kind: 'lucide', name: 'ClipboardList' }`，`dashboard-navigation.ts` 的 `icon: 'clipboardList'` 不变。
2. 其它模块：按需声明，旧字面量键不变。
...
| **resources.icons + 代码生成 + 命名空间化（本提案 v1）** | **生成期** | **好** | **好** | **低** | **一次** | **推荐** |
