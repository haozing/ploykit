...
- 插件源码发现是本地、纯代码层：已配置来源中的 `plugin.ts` 插件根会被扫描成生成模块 map。提交版默认 map 跟踪 `plugins/`；外部产品壳可以在 `.runtime/` 或 `PLOYKIT_PLUGIN_MAP_FILE` 指定路径中准备被忽略的 runtime map artifact。产品、suite 与 app bundle 归属属于安装/catalog 状态，不属于插件源码发现。
