...
- Use `scope: 'product'` for product-wide CMS or site content.
...
- Do not create session, user, or workspace tables to bypass `ctx.user`, `ctx.auth`, or `ctx.workspace`.
- Do not pass product or workspace ids as trusted client input when `ctx.scope` already carries them.
