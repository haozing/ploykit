...
对插件或模块开发者也能提速，但前提是提供 **PloyKit 原生的 admin resource 合同**，由 host 自动映射成 Refine resources。插件开发者不应默认直接 import `@refinedev/*` 或 `antd`；否则会绕开 PloyKit 的权限、审计、Data v2、ctx 能力和 UI 边界。
...
这条能力应该保留，但未来模块 admin page 应优先走 declarative admin resource，而不是每个模块手写完整后台页面。
...
4. 为模块/插件开发者提供 declarative admin resource 接口，让他们少写后台页面。
...
## 核心设计：Admin Resource Registry

新增 `AdminResourceRegistry`，替代现在分散的 page component + local filtering + server action 组合。

`AdminResourceRegistry` 不能只是 Refine 前端资源配置。它必须进入 host security catalog、admin API registry、审计和 doctor 体系。当前 `requireAdminRequestContext(request, path)` 会按 admin 页面路径查 `findAdminPageRegistryEntry(path)`，因此不能直接拿来守护 `/api/admin/resources/*`。重构时应新增 `requireAdminResourceRequestContext(request, resource, operation)`，或等价的 `executeAdminResourceOperation(...)`，由它完成 API route、安全目录、resource 定义、capability、risk 和 audit 的统一校验。

建议文件：

```text
apps/host-next/lib/admin-resource-registry.ts
apps/host-next/lib/admin-resource-handlers.ts
apps/host-next/lib/admin-resource-context.ts
apps/host-next/lib/admin-resource-execution.ts
apps/host-next/lib/admin-resource-schema.ts
apps/host-next/app/api/admin/resources/[resource]/route.ts
apps/host-next/app/api/admin/resources/[resource]/[id]/route.ts
...
```ts
interface AdminResourceDefinition<TRecord = unknown> {
  name: string;
  label: string;
  group: string;
  icon?: string;
  ...
  actions: {
    list?: AdminResourceOperation<TRecord, 'read'>;
    show?: AdminResourceOperation<TRecord, 'read'>;
    create?: AdminResourceOperation<TRecord, 'write'>;
    update?: AdminResourceOperation<TRecord, 'write'>;
    delete?: AdminResourceOperation<TRecord, 'dangerous'>;
    custom?: Record<string, AdminResourceOperation>;
  };
  auditPrefix: string;
  fields: readonly AdminResourceField[];
  table?: AdminTableDefinition<TRecord>;
}

interface AdminResourceOperation<TRecord = unknown, TRisk extends AdminRegistryRisk = AdminRegistryRisk> {
  ...
  handler: AdminResourceHandler<TRecord>;
}
```
...
- `getList` -> `GET /api/admin/resources/:resource`
- `getOne` -> `GET /api/admin/resources/:resource/:id`
- `create` -> `POST /api/admin/resources/:resource`
- `update` -> `PATCH /api/admin/resources/:resource/:id`
- `deleteOne` -> `DELETE /api/admin/resources/:resource/:id`
- `custom` -> `POST /api/admin/resources/:resource/actions/:action`
...
1. host route security catalog 能识别 `/api/admin/resources/*`，并按 admin API route 处理 CSRF、origin、rate limit 和 anonymous policy。
2. `AdminResourceRegistry` 能识别具体 resource + operation，并执行 operation 级 capability/risk/confirmation/audit。
...
前端访问控制只用于隐藏按钮和菜单。最终拒绝必须在 `/api/admin/resources/*` 服务端完成。
...
Refine + AntD 能给插件开发者提速，但不是通过“插件作者自己写 AntD 页面”来提速，而是通过 PloyKit 提供的 admin resource contract 来提速。
...
当前仓库没有 `plugins/` 目录，插件运行时也不是本次后台重构的当前代码事实。插件 admin resource 应作为后续 RFC，而不是 `/admin` Refine 化的前置条件。未来 `plugin.ts` 可以对齐同一模型：

```ts
admin: {
  resources: {
    syncJobs: pluginAdminResource({
      label: 'Sync Jobs',
      group: 'Connector',
      fields: { ... },
      handlers: {
...
POST   /api/admin/resources/users/actions/bulk-status

GET    /api/admin/resources/modules
GET    /api/admin/resources/modules/:id
POST   /api/admin/resources/modules/:id/actions/status

GET    /api/admin/resources/runs
GET    /api/admin/resources/runs/:id
POST   /api/admin/resources/runs/:id/actions/requeue
POST   /api/admin/resources/runs/:id/actions/cancel
```
...
1. `requireAdminResourceRequestContext(request, resource, operation)`
2. 找到 `AdminResourceDefinition`
...
10. 插件/模块 declarative admin resource 的自动页面生成
...
1. 新增 `AdminResourceRegistry`。
2. 新增统一 resource API route。
3. 新增 `requireAdminResourceRequestContext` 或 `executeAdminResourceOperation`。
4. 将 `/api/admin/resources/*` 纳入 host route security catalog / admin API registry。
...
### Phase 4A：模块 admin resource RFC

任务：

1. 在 module contract 中加入 declarative admin resource。
2. 从 module contracts 生成 Refine resources。
3. 增加 `module:doctor` 规则：
   - admin resource 必须声明权限。
   - mutation 必须声明 risk。
...
- 未声明权限的 admin resource 被 doctor 拒绝。

### Phase 4B：插件 admin resource RFC

任务：

1. 在 future plugin contract 中对齐 module admin resource 模型。
...
2. Admin resource API tests
   ...
   - mutation 写 audit。
   - `/api/admin/resources/*` 出现在 host route security catalog。
...
4. Module admin resource contract tests
   - 模块 admin resource 权限缺失时报 doctor error。
   - 模块 admin resource handler 越界时报 doctor error。
...
- dataProvider 只调用 `/api/admin/resources/*`。
...
Module/plugin admin     Declarative admin resources rendered by host Refine shell
...
对插件开发者来说，Refine + AntD 的最大价值不是让他们写 AntD，而是让他们通过 `plugin.ts` / `module.ts` 声明 admin resource 后，host 自动生成列表、详情、表单、动作和导航。这样既提速，又不牺牲 PloyKit 的 contract-first、安全和审计边界。
