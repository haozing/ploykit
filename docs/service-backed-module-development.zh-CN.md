...
本文面向 RunLynk 这类项目：领域内核由独立服务端维护，PloyKit 负责产品外壳、控制台、配置、权限、审计、商业化和发布门禁。目标不是把两个项目重新揉在一起，而是让双方围绕同一个机器契约并行开发，最后可验证地合并。

## 核心结论

服务端分离型项目应采用契约驱动开发：

- 服务端以 OpenAPI、AsyncAPI、Protocol Buffers 或 JSON Schema 作为机器契约源头。
- PloyKit 模块通过 `contractVersion: 2`、`serviceRequirements` 和 `ctx.services.invoke(...)` 调用受控服务。
- PloyKit module 源码放在仓库内 `modules/<id>/`；独立维护的是 Core、Worker、OpenAPI、live smoke 等服务端资产，不是 PloyKit module 壳。
...
- 文档只解释流程和语义，不能取代机器契约；如果文档和机器契约冲突，以机器契约为准。

这不是“多写一份接口文档”，而是把接口契约变成类型、mock、测试和 CI gate 的共同输入。

## 适用场景

适合使用本指南的情况：

- Go、Java、Rust、Python 等服务端独立维护权威数据和状态机。
...
共同维护 `openapi.yaml` 并由 PloyKit 生成 mock 是可行的，也应该作为服务端分离型模块的默认开发方式。推荐理解为：

```text
openapi.yaml
  -> 类型、endpoint 清单、基础 mock、consumer contract test
  -> PloyKit service client/adapter
...
生产期：baseUrl = https://core.example.com
```
...
如果 mock 试图完整模拟租约、重试、幂等、quota 和签名，它会逐渐变成另一个小服务端，并且很容易和真实服务漂移。PloyKit 应把 mock 用于开发效率，把最终正确性交给真实服务 blackbox/live smoke。
...
- 权威领域对象和状态机。
- 真实业务 API、worker/producer API、callback/webhook、usage/quota、lease/retry/cancel。
- 服务端鉴权、租户隔离、幂等、并发和数据库约束。
- OpenAPI 或等价机器契约、schema、error envelope、examples。
- 服务端黑盒测试、迁移、部署、运行时监控。
...
PloyKit 现在已经有关键基础：`contractVersion: 2`、`serviceRequirements`、`ctx.services.invoke(...)`、service connection、redaction、audit、module doctor、`module:service-contract` 和 `product + service-backed` 模板扩展。服务端分离型模块不需要推翻架构，按下面的形态落地即可；后续增强主要继续补 request/response schema diff、fixture 生成和 live evidence 自动化。
...
- `lib/service-errors.ts` 统一把服务端 error envelope 转成页面可展示、可审计、已脱敏的结构。
- `tests/smoke-contract.test.ts` 校验模块声明、service policy、权限和 wrapper 调用路径。

### 2. OpenAPI consumer check

PloyKit 提供通用检查入口，用于验证模块实际消费的服务端接口仍存在于机器契约中。
...
- 服务端删除或重命名 endpoint 时，PloyKit 模块测试立即失败。
...
对 OpenAI 或其他 AI agent 来说，mock 规则也要写进提示词：让 AI 只改 `modules/<id>/`，先读机器契约，再生成或挑选
`tests/fixtures/`，用 `createTestingModuleContext({ serviceHandlers })` 接到 `ctx.services.invoke(...)`。不要让 AI 为了 mock
在页面/action 里写 `if (mock)` 分支，也不要让 AI 把 OpenAI 生成的示例响应当成服务端权威事实。
...
service-backed 扩展：OpenAPI/受控服务/service client/mock/live smoke
...
又能叠加 OpenAPI 契约和真实服务验收。需要队列、worker、回调或长任务时，再叠加 `background`。
...
如果答案不清楚，不要急着写页面。先补服务端契约或业务边界说明。

### 2. 服务端先发布机器契约

服务端仓库至少提供：

- `openapi.yaml` 或等价机器契约。
- 统一 error envelope。
- auth schemes。
    ...
    acmeCore: {
      ...
      operations: {
        request: {
          ...
          audit: { event: 'acme.core.requested' },
          redaction: {
...
export async function invokeAcmeCore<T>(
  ctx: ModuleContext,
...
Consumer contract test
  运行 module:service-contract；校验模块消费的 method/path 没漂移，schema diff 后续在同一入口扩展。

Live smoke test
  真实 PloyKit runtime + 真实服务端；校验签名、租户、幂等、状态机。

Service blackbox test
  服务端仓库自己跑；校验 OpenAPI 路由和响应 schema。
```
...
如果机器契约、PloyKit wrapper、真实服务端三者不能同时通过，不能认为集成完成。

## RunLynk 参考落地

RunLynk 已经具备正确方向：

- Core 仓库有 `openapi.yaml`，并声明它是外部 HTTP 契约源头。
...
- Core 有 OpenAPI route/schema blackbox 和 API test matrix。
- Worker contract endpoint 返回 schema、capability、endpoint template、starter defaults 和 `mock_input`。

还可以继续补强：

- 在 PloyKit 侧接入 `module:service-contract`，确保 `modules/runlynk/lib/core-*` 使用的 endpoint 都存在于 `openapi.yaml`。
...
- 服务端接口变更只改 Go handler，不更新 OpenAPI。
- PloyKit 页面变更只改 mock，不跑真实 service smoke。
...
| 服务端已有 OpenAPI | OpenAPI 作为机器契约，生成类型、mock、consumer check |
| 服务端还没有机器契约 | 先补机器契约，再写 PloyKit 页面 |
