...
AI 可以写 PloyKit 模块，但必须让它在本地模块边界里工作：只改配置的模块根目录，从 `module.ts` 开始理解契约，按 doctor 诊断循环修复。

如果任务只是模块接入，默认写权限只开放配置的模块根目录、模块本地生成文件和测试，以及 module-map 生成文件；不要修改 `apps/host-next/*`、`src/lib/module-runtime/*`、`src/module-sdk/*`、`scripts/host-*`。确实缺少宿主扩展点时，不要用 `moduleId === '<id>'`、`import modules/<id>`、硬编码 `<module-root>` 或把模块路由塞进宿主脚本来完成需求；先报告需要通用 registry/contribution seam，再做宿主级抽象。

## 推荐提示词

```text
你正在开发 PloyKit 本地模块。只修改配置的模块根目录。
先阅读 module.ts，确认 routes/actions/jobs/events/webhooks/data/permissions。
默认只修改配置的模块根目录；除 module-map 生成文件外，不把模块验收改成宿主全局门禁。
